...
void creat_tree(Processtree *root, int tab_length) {
  int flag = 2;
  char str[100] = {0};
  int allchildrensindex[500] = {
      0}; // Log the index of all root child processes in Pidinfos
  findallchildrens(root->pid, allchildrensindex);
  root->children =
      (struct childprocesses *)malloc(sizeof(struct childprocesses));
  ...
  for (int i = 0; i < 500 && allchildrensindex[i] != 0;
       i++) // Traverses all children of the process
  {
...
int main(int argc, char *argv[]) {
  ...
  if (!ops) {
    ...
    {
      printf(
          "pstree (PSmisc) UNKNOWN\nCopyright (C) 1993-2019 Werner Almesberger "
