...
int linkedlist_insert(char *procname, pid_t pid, pid_t ppid)
{
	...
	strcpy(node->name, trim(procname));
	node->pid = pid;
	node->ppid = ppid;
...
// 				   len + k); // print the children node
// 			i += 1;
// 			printf("\n");
// 			for (int l = 0; l < len; ++l) {
// 				if (i > 0)
// 					printf(" "); // try to make sure the children process are in the same
...
int print_tree(struct pstree_node *root, int len)
{
	...
	if (root->numofC == 0) {
		...
		for (int i = root->numofC - 1; i >= 0; i--) {
			...
			print_tree(root->children[i], newlen);
		}
	}
	return 0;
}
