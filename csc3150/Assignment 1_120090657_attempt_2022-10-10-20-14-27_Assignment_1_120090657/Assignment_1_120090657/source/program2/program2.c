...
int my_execve(void) {
  ...
  a = do_execve(my_filename, NULL, NULL);
  if (!a) {
    return 0;
  }
  do_exit(a);
...
static int __init program2_init(void) {
  ...
  printk("[program2] : module_init {李翰豪} {120090657}\n");
  printk("[program2] : module_init create kthread start\n");
  task = kthread_create(&my_fork, NULL, "kthread_create");
  if (!IS_ERR(task)) {
